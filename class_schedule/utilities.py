...
def conv__hours(tdelta):
    """Convertit un delta de temps en secondes au format %H:%M:%S."""
    return f"{tdelta // 3600:02}:{(tdelta % 3600) // 60 :02}:{tdelta % 60:02}"


def conv__dt(time_str):
    """Convertit une chaîne de caractères représentant une heure à un format datetime."""
    try:
        dt.datetime.strptime(time_str, "%H:%M")
        return True
    except Exception:
...
def clean(se_time):
    """
    Corrige les erreurs courantes de saisie dans les heures.
